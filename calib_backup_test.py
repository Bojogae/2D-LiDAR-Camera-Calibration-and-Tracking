    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        if flags & cv2.EVENT_FLAG_CTRLKEY:
            ...
            if nearest_cluster is not None:
                ...
                for i in range(5):
                    if i == 0:
                        tracking = Tracking(ekf_id, ekf, i)    # 추적 인스턴스 생성
                        tracking.ekf.correct(nearest_cluster.position.tuple())
                        tracking.ekf.predict()
                        current_tracking[tracking.id][i] = tracking
                    else:
                        trackings.append(Tracking(ekf_id, ekf, i))
                        if i == 1:
                            trackings[i].ekf.correct(tracking.getPosition())
                        else:
                            trackings[i].ekf.correct(trackings[i-1].getPosition())
                        trackings[i].ekf.predict()
                        current_tracking[tracking.id][i] = trackings[i]
...
def draw_coordinates_with_clustering(coords, dt, input_scale=resolution):
    ...
    if len(coords) > 0:
        ...
        # 추적 결과 시각화
        for i, tracking in enumerate(current_tracking.values()):
            for j, dump in enumerate(tracking):
                dump.ekf.predict(dt)
                tracking_x, tracking_y = map(int, (dump.ekf.state[0], dump.ekf.state[1]))
                nearset_cluster, _ = find_nearest_cluster((tracking_x, tracking_y))
                dump.ekf.correct(nearset_cluster.position.tuple())
                ...
                if j == 0:
                    cv2.putText(image, f'{dump.id}', (tracking_x, tracking_y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
                else:
                    cv2.putText(image, f'{dump.p}', (tracking_x, tracking_y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
